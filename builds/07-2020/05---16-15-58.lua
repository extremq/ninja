...
function generateProfileImgs(playerName, target)
    ...
    if pageId:find("@") then
        tfm.exec.chatMessage("extrmq sucksxdx")
        profileName = pageId:match("@(%w+#%d+)")
    end
    local playerTag = profileName:match("#%d%d%d%d")
    local colorTag = 'V'
    if playerTag == "#0010" or playerTag == "#0000" then
        colorTag = 'J'
    ...
    addTextArea(17, "<font face='Soopafresh,Verdana' size='30'><"..colorTag..">"..removeTag(profileName).."</v> <font size='-5'><"..colortag..">"..playerTag.." </g></font></font>", playerName, 126, 80, 438, 45, 0x324650, 0x000000, 0, true)
