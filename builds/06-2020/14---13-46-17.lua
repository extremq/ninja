...
shop = {
    dashAcc = {
        ...
        shopListing({3, 31}, "172a5639431.png", "Add some hearts to your dash!", "Finish 10 maps.", {"mapsFinished", 10}),
...
function generateShopPartImgsText(playerName, pageNumber)
    ...
    for i = 1, 3 do
        ...
        elseif shop.dashAcc[(pageNumber - 1) * 3 + i].fnc(playerName) == false then
            ...
            local selectState = "<a href='event:SelectDash"..i.."'><font size='11'>[Select]</font></a>"
            if playerStats[playerName].equipment[1] == (pageNumber - 1) * 3 + i then
                selectState = "<V><a href='event:nil'>[Selected]</a></V>"
            end
...
function eventTextAreaCallback(textAreaId, playerName, eventName)
    ...
    elseif string.find(currentPage, "shopPart") then
        ...
        if eventName == "NextPage" and currentPageNumber + 1 <= maxPages then
            openPage(translate(playerName, "shopTitle"), generateShopPage(playerName, currentPageNumber + 1), playerName, "shopPart#"..(currentPageNumber + 1))
        ...
        if string.find(eventName, "SelectDash") then
            ...
            openPage(translate(playerName, "shopTitle"), generateShopPage(playerName, currentPageNumber), playerName, "shopPart#"..currentPageNumber)
        end
    end
