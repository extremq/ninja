...
function updateMapName(timeRemaining)
    ...
    if fastestplayer ~= -1 then
        ...
        name = name.." <G>|<G> <N2>Record: </N2><modRoom>"..fastestplayer.." - "..record.."s</modRoom>"
    end
    ...
    elseif elapsedTime >= MAPTIME * 1000 + 5000 or mapWasSkipped == true then
        ...
        for playerName in pairs(room.playerList) do
            ...
            if id ~= 0 then
                ...
                if cooldowns[id].canRewind == true and rewindstate[id] ~= 2 then
                    rewindstate[id] = 2
                    removeImage(rewindbtnid[id])
                    rewindbtnid[id] = addImage(REWIND_BTN_ACTIVE, "&1", REWIND_BTN_X, REWIND_BTN_Y, playerName)
                elseif cooldowns[id].canRewind == false and rewindstate[id] ~= 1 and ostime - cooldowns[id].lastRewindTime > REWINDCOOLDONW then
                    rewindstate[id] = 1
                    removeImage(rewindbtnid[id])
                    rewindbtnid[id] = addImage(REWIND_BTN_ON, "&1", REWIND_BTN_X, REWIND_BTN_Y, playerName)
                elseif rewindstate[id] ~= 3 and ostime - cooldowns[id].lastRewindTime <= REWINDCOOLDONW then
                    rewindstate[id] = 3
                    removeImage(rewindbtnid[id])
                    rewindbtnid[id] = addImage(REWIND_BTN_OFF, "&1", REWIND_BTN_X, REWIND_BTN_Y, playerName)
                end
...
function eventPlayerWon(playerName, timeElapsed, timeElapsedSinceRespawn)
    ...
    ui.updateTextArea(4, "<p align='center'><font face='Lucida console' color='#ffffff'>"..translations[playerLanguage[id]].lastBestTime..": "..(playerVars[id].playerBestTime/100).."s", playerName)
...
function initPlayer(playerName)
    ...
    if playerVars[id].playerBestTime == nil then
        playerVars[id].playerBestTime = 999999
        playerVars[id].playerLastTime = 999999
        ...
        playerVars[id].playerpreferences = {false, true, false}
    else 
        local hasFound = false
        for key, value in pairs(playerSortedBestTime) do
            if value[1] == playerName then
                playerVars[id].playerBestTime = value[2]
                playerVars[id].playerLastTime = value[2]
                hasFound = true
                break
            end
        end
    ...
    if playerVars[id].playerpreferences[3] == true then
        ...
        if playerVars[id].playerFinished == true then
            ui.updateTextArea(5, "<p align='center'><font face='Lucida console' color='#ffffff'>"..translations[playerLanguage[id]].lastTime..": "..(playerVars[id].playerLastTime/100).."s", playerName)
            ui.updateTextArea(4, "<p align='center'><font face='Lucida console' color='#ffffff'>"..translations[playerLanguage[id]].lastBestTime..": "..(playerVars[id].playerBestTime/100).."s", playerName)
        end
    end
end
...
function extractMapDimensions()
    ...
    local x = string.match(p, 'modList="(%d+)"')
    if x == nil then
        return 800
    end
    return tonumber(x)
...
function stats(playerName, playerId, creatorId)
    ...
    body = body.." » "..translations[playerLanguage[creatorId]].firsts..": <modRoom>"..playerStats[playerId].mapsFinishedFirst.."</modRoom>\n"
    body = body.." » "..translations[playerLanguage[creatorId]].finishedMaps..": <modRoom>"..playerStats[playerId].mapsFinished.."</modRoom>\n"
    local firstrate = "0%"
    ...
    body = body.." » "..translations[playerLanguage[creatorId]].firstRate..": <modRoom>"..firstrate.."</modRoom>\n"
    body = body.." » "..translations[playerLanguage[creatorId]].holeEnters..": <modRoom>"..playerStats[playerId].timesEnteredInHole.."</modRoom>\n"
    body = body.." » "..translations[playerLanguage[creatorId]].graffitiUses..": <modRoom>"..playerStats[playerId].graffitiSprays.."</modRoom>\n"
    body = body.." » "..translations[playerLanguage[creatorId]].dashUses..": <modRoom>"..playerStats[playerId].timesDashed.."</modRoom>\n"
    body = body.." » "..translations[playerLanguage[creatorId]].rewindUses..": <modRoom>"..playerStats[playerId].timesRewinded.."</modRoom>\n"
