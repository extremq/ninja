...
do
    function translate(playerName, what, ...)
        ...
        if #playerName == 2 then
            local language = translations.playerName or translations.en
        else
...
function generatedashAccImgsText(playerName, pageNumber)
    ...
    for i = 1, 3 do
        shopAccVar = shop.dashAcc[sortedOrder[(pageNumber - 1) * 3 + i]]
        if shopAccVar.reqs[2] == nil then
            local reqs = translate(playerName, shopAccVar.reqs[1])
        else
            local reqs = translate(playerName, shopAccVar.reqs[1], shopAccVar.reqs[2])
        end
        if shopAccVar == nil then
