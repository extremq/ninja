...
function generateShopPage(playerName, pageNumber)
    local body = "<font face='Lucida Console' size='16'>\n\n\n\n\n\n\n\n\n\n\n\n\n\n\n<p align='center'><a href='event:PrevPage'>............&lt;</a> "..pageNumber.." / "..math.ceil(#shop.dashAcc / 3).." <a href='event:NextPage'>&gt;............</a></p>"
    return body
end
...
function generateShopPartImgsText(playerName, pageNumber)
    ...
    for i = 1, 3 do
        if shop.dashAcc[(pageNumber - 1) * 3 + i] == nil then
            ...
            addTextArea(ids[i], "<p align='center'><i><CS>Coming soon!</CS></i></p>", playerName, x[i], 200, 100, 100, 0x0a1517, 0x122529, 1, true)
...
function eventTextAreaCallback(textAreaId, playerName, eventName)
    ...
    elseif currentPage == "shop" then
        if eventName == "ChangePart" then
            openPage(translate(playerName, "shopTitle"), generateShopPage(playerName, 1), playerName, "shopPart#1")
        end
    elseif string.find(currentPage, "shopPart") then
        local maxPages = math.ceil(#shop.dashAcc / 3)
        local nmbr = tonumber(string.match(currentPage, "%d+"))
        if eventName == "NextPage" and nmbr + 1 <= maxPages then
            openPage(translate(playerName, "shopTitle"), generateShopPage(playerName, nmbr + 1), playerName, "shopPart#"..(nmbr + 1))
        elseif eventName == "PrevPage" and tonumber(nmbr) - 1 > 0 then
            openPage(translate(playerName, "shopTitle"), generateShopPage(playerName, nmbr - 1), playerName, "shopPart#"..(nmbr - 1))
        end

        if string.find(eventName, "SelectDash") then
            local selectedNmbr = tonumber(string.match(selectedNmbr, "%d+"))
            playerStats[playerName].equipment[1] = selectedNmbr
            openPage(translate(playerName, "shopTitle"), generateShopPage(playerName, nbmr), playerName, "shopPart#"..nmbr)
