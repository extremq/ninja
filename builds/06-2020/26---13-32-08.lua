    ...
    for i = 1, 5 do
        ...
        elseif currentShopItem.fnc(playerName) == false then
            ...
            elseif type == "graffitiFonts" then
                addTextArea(ids[i] + 20, "", playerName, imgBgX, y[i] - 3, 40, 40, 0x264E57, 0x264E57, 1, true)
                addTextArea(ids[i] + 21, "<p align='center'><font face='"..shop.graffitiFonts[sortedOrder[(pageNumber - 1) * 5 + i]].imgId.."' size='12' color='"..shop.graffitiCol[playerStats[playerName].equipment[2]].values.."'>Abc</font></p>", playerName, imgBgX, y[i] - 10, 40, 40, 0x264E57, 0x264E57, , true)
            end
        else
            local selectState = "<a href='event:Select"..sortedOrder[(pageNumber - 1) * 5 + i].."'>"..translate(playerName, currentShopItem.tooltip).."</a>"
            local index = 2
