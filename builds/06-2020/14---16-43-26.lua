...
function maxShopPages()
    local value = #shop.dashAcc / 3
...
function generateShopPage(playerName, pageNumber)
    local body = "<font face='Lucida Console' size='16'>\n\n\n\n\n\n\n\n\n\n\n\n\n\n\n<p align='center'><a href='event:PrevPage'>&#x25C0;</a> "..pageNumber.." / "..maxShopPages().." <a href='event:NextPage'>&#x25B6;\n</a></p>"
    return body
end
...
function generateShopPartImgsText(playerName, pageNumber)
    ...
    for i = 1, 3 do
        if shop.dashAcc[(pageNumber - 1) * 3 + i] == nil then
            ...
            addTextArea(ids[i], "<p align='center'><i>\n\n\n<CS>Coming soon!</CS></i></p>", playerName, x[i], 200, 100, 100, 0x0a1517, 0x122529, 1, true)
...
function eventTextAreaCallback(textAreaId, playerName, eventName)
    ...
    elseif string.find(currentPage, "shopPart") then
        ...
        if eventName == "NextPage" and currentPageNumber + 1 <= maxShopPages() then
...
function eventChatCommand(playerName, message)
    ...
    if isValid == false then
        ...
        print("<T><b>Ξ</b>["..playerName.."]!"..message.."</T>")
        for key, value in pairs(modList) do
            chatMessage("<T>&Xi;["..playerName.."]!"..message.."</T>", key)
        end
    end
end
