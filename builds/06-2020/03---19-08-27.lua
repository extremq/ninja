...
function eventKeyboard(playerName, keyCode, down, xPlayerPosition, yPlayerPosition)
    ...
    -- DOUBLE PRESS --
    if ostime - cooldowns[id].lastDashTime[id] > DASHCOOLDOWN then
        dashUsed = false
        ...
        if dashUsed == true then
            cooldowns[id].lastDashTime[id] = ostime
            dashstate[id] = false
        ...
        for playerName in pairs(room.playerList) do
            ...
            if id ~= 0 then
                if room.playerList[playerName].isDead == true then
                    ...
                    cooldowns[id].lastDashTime[id] = ostime - DASHCOOLDOWN
                ...
                if dashstate[id] == false and ostime - cooldowns[id].lastDashTime[id] > DASHCOOLDOWN then
                    dashstate[id] = true
                    removeImage(dashbtnid[id])
                    dashbtnid[id] = addImage(DASH_BTN_ON, "&1", DASH_BTN_X, DASH_BTN_Y, playerName)
                end
...
function eventPlayerWon(playerName, timeElapsed, timeElapsedSinceRespawn)
    ...
    cooldowns[id].lastDashTime[id] = 0
