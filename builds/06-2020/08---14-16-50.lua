...
function eventPlayerWon(playerName, timeElapsed, timeElapsedSinceRespawn)
    ...
    ui.updateTextArea(4, "<p align='center'><font face='Lucida console' color='#ffffff'>"..translations[playerVars[playerName].playerLanguage].lastBestTime..": "..(playerVars[id].playerBestTime/100).."s", playerName)
...
function eventTextAreaCallback(textAreaId, playerName, eventName)
    ...
    if textAreaId == 12 then
        ...
        if eventName == "StatsOpen" then
            if playerVars[playerName].menuPage == 0 then
                createPage(translations[playerVars[playerName].playerLanguage].profileTitle.." - "..playerName, stats(playerName, id, id), playerName, "profile")
            else
                updatePage(translations[playerVars[playerName].playerLanguage].profileTitle.." - "..playerName, stats(playerName, id, id), playerName, "profile")
            end
        end
    ...
    if arg[1] == "p" or arg[1] == "profile" then
        ...
        if arg[2] == nil then
            if playerVars[id].menuPage == 0 then
                createPage(translations[playerVars[playerName].playerLanguage].profileTitle.." - "..playerName, stats(playerName, room.playerList[playerName].id, id), playerName, id, "profile")
            else
                updatePage(translations[playerVars[playerName].playerLanguage].profileTitle.." - "..playerName, stats(playerName, room.playerList[playerName].id, id), playerName, id, "profile")
            end
            return
        end

        for name, value in pairs(room.playerList) do
            if name == arg[2] and value.id ~= 0 then
                if playerVars[id].menuPage == 0 then
                    createPage(translations[playerVars[playerName].playerLanguage].profileTitle.." - "..arg[2], stats(arg[2], room.playerList[arg[2]].id, id), playerName, id, "profile")
                else
                    updatePage(translations[playerVars[playerName].playerLanguage].profileTitle.." - "..arg[2], stats(arg[2], room.playerList[arg[2]].id, id), playerName, id, "profile")
                end
                break
            end
        end
