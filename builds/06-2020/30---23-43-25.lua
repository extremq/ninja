...
translations.en = {
    ...
    enoughPlayers = "<CH>[•] There are %s players in this room and stats are counted!</CH>",
    enoughPlayers = "<CH>[•] More players are needed for stats to count. [%s/3]</CH>",
    statsCount = "<CH>[•] Stats are now counted!</CH>",
    statsDontCOunt = "<CH>[•] Stats are not counted anymore!</CH>",
...
function updateMapName(timeRemaining)
    ...
    local name = currentmapauthor.." <G>-</G><N> "..currentmapcode.."</N> <G>-</G> "..translate(roomCommunity, "infobarLevel").." "..difficultyMessage.." <G>|<G> <N>"..translate(roomCommunity, "infobarMice").."</N> <J>"..tfm.get.room.uniquePlayers..."</J> <G>|<G> <N>"..minutes..":"..seconds.."</N>"
    -- Append record
...
function putInClearQueue(what, where, playerName)
    table.insert(queue[playerName][where], what)
end
...
function addBackButton(playerName)
    putInClearQueue(addImage(BACK_BTN, ":101", 542, 55, playerName), "img", playerName)
    addTextArea(16, "<a href='event:Back'>\n</a>", playerName, 540, 53, 20, 20, 0x324650, 0x000000, 0, true) 
...
function generateShopImgs(playerName)
    ...
    putInClearQueue(addImage(shop.dashAcc[playerStats[playerName].equipment[1]].imgId, "&2", dashX, dashY, playerName), "img", playerName)
    ...
    putInClearQueue(id + graffitiTextOffset, "area", playerName)
end
...
function generatedashAccImgsText(playerName, pageNumber)
    ...
    for i = 1, 5 do
        ...
        if currentShopItem == nil then
            putInClearQueue(addImage(FORBIDDEN, "&"..i, imgBgX, y[i] - 2, playerName), "img", playerName)
            putInClearQueue(addImage(LOCK, "&"..i, imgBgX + statusOffset, y[i] + statusOffset, playerName), "img", playerName)
            addTextArea(ids[i], "<i><CS>"..translate(playerName, "comingSoon").."</CS></i>", playerName, x, y[i], 315, 33, 0x0a1517, 0x122529, 0, true)
        ...
        putInClearQueue(ids[i], "area", playerName)
        addTextArea(ids[i] + 20, "", playerName, imgBgX, y[i] - 3, 40, 40, 0x264E57, 0x264E57, 1, true)
        putInClearQueue(ids[i] + 20, "area", playerName)
    end
end
